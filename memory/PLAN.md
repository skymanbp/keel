...
8. [x] **Ship keel-onnx 0.1 with the end-to-end train-in-Python run-on-Windows demo** — DONE (local dev) 2026-08-18: demo passes on Windows AND in CI on all 3 OSes (regression + classifier, agreement 0.0, gate 1e-6, names/shapes via introspection); 31 slot-gated OrtApi bindings vs vendored v1.24.4 header; zero-copy BOTH ways (input borrow + output ForeignPtr views releasing OrtValues); int64/double/float typed outputs; TensorInfo introspection; leak gate flat (-24 KiB over 500 inferences); stray PATH ORT 1.17.1 exercised OnnxApiUnsupported for real. Hackage upload at publish stage (step 11)
...
11. [~] **Ship the keel umbrella, doctor, setup, docs, Stackage entry and announcement**  ← ACTIVE — OUTWARD UNLOCKED 2026-08-18. Done: public repo github.com/skymanbp/keel; 3-OS x 3-GHC CI GREEN 9/9 (run 32143830572, incl. 9.10.3 floor + arm64; per-suite audit: pyarrow/DLPack/ONNX-demo genuinely ran on all OSes); keel doctor (diagnosed real machine issues on first run); keel setup blas+onnx SHA-256-pinned official archives PROVEN live (this machine went 2/4 -> 4/4 'all capabilities available'; data-dir 1.24.4 outranks stray PATH 1.17.1); REQUIRE gates on (SKIP on CI = failure); CI Windows runs 'keel setup blas' each round = e2e setup test. 2026-08-18 later: Keel.Bridge landed (ONLY dataframe-core contact point; honest O(n) copies, nulls refused; discovery: dataframe pads short columns with nulls -> asserted) + 4 tutorials in docs/tutorials/ (code drawn from CI-verified tests) + README full-matrix status. Remaining: release polish (versions/changelogs/sdist check), Hackage batch (BLOCKED: needs owner's Hackage credentials; candidates first as the no-regret path), Stackage nightly, Discourse + design partner
...
- Active step: #11
- Last refined: 2026-08-18T10:25:31 (plan-refiner)
- Last guardian check: 2026-08-18T10:07:15
